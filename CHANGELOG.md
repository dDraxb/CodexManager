...
### Added
- Added provider-neutral external session identity fields and API support while mirroring Codex history fields for compatibility.
...
- Adopt and resume-point UI copy now uses provider session terminology instead of Codex-only session identity labels.
